...
public extension Intervals {
    ...
    private var chordBasicCase: ChordBasicCase? {
        ...
        let intervalsOmitTensionRoot: Set<Interval> = filter { $0.degreeInt != 1 && $0.degreeInt <= 7 }
    ...
    internal var symbolRawSequence: [ChordSymbol]? {
        ...
        guard let chordBasicCase = chordBasicCase else { return nil }

        // - 获取预设中定义好的基底
        let symbolShells: [ChordSymbol] = chordBasicCase.symbolShells

        // - 过滤并提取出当前和弦含有所有的张力音程（度数 > 7），以从小到大的顺序排列
        let intervalsTension = filter { $0.degreeInt > 7 }.sorted()
    ...
    var symbolsRephrasedSequence: [ChordSymbol] {
