...
//    }
//    
//    /// 获取当前和弦对应的所有 ChordSymbol 集合。
//    /// - 建议 1 的回答：由于 `symbolGroups` 本身就是一维数组 `[ChordSymbol]?`，没有二维嵌套结构，使用 `flat` 会引起误解。推荐使用 `allSymbols`（或 `symbols`）能更加直白地体现“该和弦所有可用的符号组合”。
//    /// - 建议 2 的回答：返回 `[]`（空数组）比返回 `nil` 更好。在 Swift 中返回空数组可以直接配合 `.map`、`.filter` 或 `for-in` 循环使用，避免了调用方需要额外去做 `if let` 的可选解包操作。
//    public var allSymbols: [ChordSymbol] {
//        return symbolGroups ?? []
//    }
//    
//}
